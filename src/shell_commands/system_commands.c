// ==============================================================================
// NEW SYSTEM UTILITIES - Added for system maturity
// ==============================================================================
...
void quota_command(int argc, char **argv) {
    if (argc < 2) {
        ...
        printf("  quota -a             - Show quotas for all users\n");
        return;
    }
    
    if (strcmp(argv[1], "-a") == 0) {
        printf("Showing quotas for all users (feature not yet implemented)\n");
        return;
    }
    ...
    if (disk_get_quota(username, &quota) == 0) {
        char used_buf[64], limit_buf[64];
        
        printf("=== Disk Quota for %s ===\n", username);
        printf("Block Usage:    %s / %s\n", 
               disk_format_size(quota.block_used, used_buf, sizeof(used_buf)),
               disk_format_size(quota.block_hard_limit, limit_buf, sizeof(limit_buf)));
        printf("Inode Usage:    %llu / %llu\n",
               (unsigned long long)quota.inode_used,
               (unsigned long long)quota.inode_hard_limit);
        
        if (disk_check_quota_exceeded(username)) {
